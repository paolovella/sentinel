...
//! `src/trusted_merkle_hash.rs` and `PARITY-HAND-2` in
...
fn main() {
    ...
    let extraction = Path::new(&manifest_dir).join("../formal/kani/src/merkle_sanity.rs");
    println!("cargo:rerun-if-changed={}", extraction.display());
    materialize(
        &extraction,
        &Path::new(&out_dir).join("kani_merkle_sanity_extraction.rs"),
    );
}
