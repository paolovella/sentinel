...
//! - Intent scope ↔ vellaveto-types/src/channel_separation.rs IntentScopeConfig
//! - Scope enforcement ↔ vellaveto-proxy/src/main.rs scope checks
//!
//! To verify:
...
pub struct ScopeState {
    /// Bitmask of allowed sink classes (up to 8 classes).
    pub allowed_mask: u8,
...
pub open spec fn spec_in_scope(allowed_mask: u8, sink_bit: u8) -> bool {
    sink_bit < 8 && (allowed_mask >> sink_bit) & 1u8 == 1u8
}

/// Spec: restrict scope by intersecting with a restriction mask.
/// Result is always a subset of the current mask.
pub open spec fn spec_restrict_scope(current_mask: u8, restriction_mask: u8) -> u8 {
    (current_mask & restriction_mask) as u8
}

/// Spec: scope narrowing — restricted mask is subset of current.
pub open spec fn spec_is_subset_mask(restricted: u8, original: u8) -> bool {
    (restricted & original) == restricted
}

// ── SCOPE-1: Restriction is subset of original ────────────────────

pub fn restrict_scope(current_mask: u8, restriction_mask: u8) -> (result: u8)
    ensures
...
pub proof fn lemma_restriction_is_subset(current_mask: u8, restriction_mask: u8)
    ensures
        spec_is_subset_mask(
...
pub fn attempt_scope_expansion(state: &ScopeState, new_sink_bit: u8) -> (result: u8)
    requires
        new_sink_bit < 8,
    ensures
        state.locked ==> result == state.allowed_mask,
        !state.locked ==> result == (state.allowed_mask | (1u8 << new_sink_bit)),
{
    if state.locked {
        state.allowed_mask
    } else {
        state.allowed_mask | (1u8 << new_sink_bit)
    }
}
