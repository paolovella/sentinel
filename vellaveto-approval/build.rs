...
//! `src/kani_approval_drift_differential.rs` and `PARITY-HAND-2` in
...
fn main() {
    ...
    let extraction = Path::new(&manifest_dir).join("../formal/kani/src/approval_drift.rs");
    println!("cargo:rerun-if-changed={}", extraction.display());
    materialize(
        &extraction,
        &Path::new(&out_dir).join("kani_approval_drift_extraction.rs"),
    );
}
